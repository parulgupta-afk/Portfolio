...
export const MissionTimeline: React.FC = () => {
  ...
  return (
    <section id="mission-log" className="relative z-10 px-4 sm:px-8 md:px-12 lg:px-16 py-12 max-w-[1440px] mx-auto">
      ...
      <div className="border-l border-[#4cd9e0]/30 ml-3 space-y-6">
        {EXPERIENCE_DATA.map((e) => (
          <div key={e.id} className="pl-6 relative">
            ...
            >
              <p className="text-[10px] font-mono-custom text-[#8f9195]">{e.period} · {e.status}</p>
            ...
            {open === e.id && (
              <div className="mt-3 glass-panel-subtle rounded-xl p-4 space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5 pt-2">
                  {e.techStack.map((t) => (
                    <span key={t} className="text-[10px] border border-white/10 px-2 py-0.5 rounded text-[#8f9195]">
                      {t}
                    </span>
                  ))}
                </div>
